...
- **Приходы (ГРУППЫ)** — split-вид: список приходов слева; по клику приход
...
- **Продукты (ПРИХОД / ПРОДУКТЫ)** — список всех продуктов с **фильтром по типу**
  и по спецификации.
...
| ---------------- | ------------ | ------------------------------------------------------------- |
| **ПРИХОД**       | `/income`    | Список продуктов (Продукты / N)                               |
...
к которому подключается frontend.
...
- Frontend — http://localhost:5173 (отдаётся через nginx)
- Backend — http://localhost:3000

Браузер обращается к backend напрямую по `http://localhost:3000`, поэтому порт
backend публикуется наружу (это уже настроено в `docker-compose.yml`).
